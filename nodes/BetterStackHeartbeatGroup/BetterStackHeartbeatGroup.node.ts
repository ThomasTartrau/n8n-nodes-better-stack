...
function executeGetMany(
	...
	if (returnAll) {
		return client
			.requestAllPages<GroupAttributes>(
	...
	return client
		.request<GroupAttributes>(
			ApiRequestFactory.heartbeatGroups.list({ per_page: limit }),
		)
...
function executeGet(
	...
	return client
		.request<GroupAttributes>(ApiRequestFactory.heartbeatGroups.get(groupId))
...
function executeCreate(
	...
	return client
		.request<GroupAttributes>(ApiRequestFactory.heartbeatGroups.create(body))
		.then((response) => transformJsonApiResponse(response));
}
...
function executeUpdate(
	...
	return client
		.request<GroupAttributes>(
			ApiRequestFactory.heartbeatGroups.update(groupId, body),
		)
		.then((response) => transformJsonApiResponse(response));
}
...
function executeDelete(
	...
	return client
		.requestSimple(ApiRequestFactory.heartbeatGroups.delete(groupId))
		.then(() => [
			{
				success: true,
				id: groupId,
...
function executeGetHeartbeats(
	...
	if (returnAll) {
		return client
			.requestAllPages(
				ApiRequestFactory.heartbeatGroups.listHeartbeats(groupId),
			)
			.then((resources) => resources.map((r) => flattenJsonApiResource(r)));
	}
...
function executeOperation(
	...
	switch (operation) {
		...
		case "getHeartbeats":
			return executeGetHeartbeats(context, client, itemIndex);
		default:
			return Promise.reject(new Error(`Unknown operation: ${operation}`));
	}
}

export class BetterStackHeartbeatGroup implements INodeType {
	description: INodeTypeDescription = {
		displayName: "Better Stack Heartbeat Group",
		name: "betterStackHeartbeatGroup",
		icon: "file:../../icons/betterstack.svg",
		group: ["transform"],
		version: 1,
		subtitle: '={{$parameter["operation"]}}',
		description: "Manage Better Stack Uptime heartbeat groups",
		defaults: {
			name: "Better Stack Heartbeat Group",
		},
		inputs: ["main"],
		outputs: ["main"],
		usableAsTool: true,
